    ...
    for (size_t i = 0; i < frame_data_.size(); i++) {
        for (const auto &cmds : cmds_vec) {
            if (cmds == cmds_vec.back()) {
				...
				frame_data_[i].worker_microprofile.push_back(0);
            }
        }
    }
...
{
	MICROPROFILE_SCOPEI("Hologram", "update_simulation", MP_GREENYELLOW);
    sim_.update(worker.tick_interval_, worker.object_begin_, worker.object_end_);
}

void Hologram::draw_objects(Worker &worker)
{
	MICROPROFILE_SCOPEI("Hologram", "draw_objects", MP_ALICEBLUE);
	...
	MICROPROFILE_GPU_BEGIN(cmd, pGpuLog);
	{
		MICROPROFILE_SCOPEGPUI_L(pGpuLog, "DrawThread", MP_DARKGOLDENROD);
		...
		{
			MICROPROFILE_SCOPEGPUI_L(pGpuLog, "DrawLoop", MP_HOTPINK3);
			for (int i = worker.object_begin_; i < worker.object_end_; i++) {
				//MICROPROFILE_SCOPEGPUI_L(pGpuLog, "DrawOne", MP_HOTPINK);
	...
	MICROPROFILE_CONDITIONAL(data.worker_microprofile[worker.index_] = MICROPROFILE_GPU_END(pGpuLog) );
...
void Hologram::on_frame(float frame_pred)
{
	...
	for(uint32_t i = 0; i < data.worker_microprofile.size(); ++i)
	{
		MICROPROFILE_GPU_SUBMIT(MicroProfileGetGlobalGpuQueue(), data.worker_microprofile[i]);
		data.worker_microprofile[i] = 0;
	}


    vk::CmdEndRenderPass(data.primary_cmd);
	MicroProfileFlip(data.primary_cmd);
